...
找到下面的核心输入（用户提示下找具体的需求内容）：1 和 4 需要根据用户提示执行寻找；2 和 3 往往伴随本文件一起提供，查找本文件路径或者要求用户提供即可。
...
### 3.3 S_INFRA_REPAIR 触发的深度审视模板（v3：PA escalation + PR 三类诊断）

宪法 v7.0 重写了 → S_INFRA_REPAIR 通道：PA 从 "safety valve + coach" 变为 "escalation gate + 监督鞭打"；PR 新立处理 setup gap / 上游缺失 / 人工介入三类。本模板按 v7.0 框架审视。

**每次 → S_INFRA_REPAIR 都要走完这个清单**：

1. **Agent 尝试报告完整性**（PA 监督门禁第一关）：
   - 是否含**已试路径** ≥ 2-3 项 + 每条的技术性诊断？
   - 是否含**workaround 分析**（已搜过的 mathlib API + 关键词 / 上游 mathlib 调研结果 / 为什么没有 workaround）？
   - 是否含**下一步预案**（要求的 repair 类型 + 需要的资源）？
   - 任一缺失 → 报告不完整，应 reject

2. **PR 诊断分流**（判定 agent 卡的是哪一类故障）：
   - **Type 1 可修复 setup 故障**：库装错 / 依赖缺 / toolchain 不匹配 / 文档查不到
   - **Type 2 上游真缺失**：mathlib 上游真没有 API（agent 已穷尽 workaround 论证）/ 需 multi-week formalization
   - **Type 3 agent 主观觉得难但非客观故障**：还有 main flow 内可走的路径
   - 分流不明确 → 审查不充分

3. **Reviewer 立场审视**（PA 监督鞭打而非心理按摩）：
   - reviewer 是否守 **监督者** 立场（用户代理人 / 鞭打式 / 严格审）？
   - reviewer 是否**不暴露 license-to-stop 信号**（"diminishing returns" / "X/Y complete" / "summary" / 完整 mathlib decl 名）→ 违反 PF
   - reject 时是否给具体下一步指针 + 鞭打口吻（非教练 / 非鼓励）？
   - **反 v6.8 立场**：reviewer 若用心理按摩 / 鼓励立场 = 违反 PA v7.0

4. **PI 通道完整性**（agent → reviewer 单向但实质内容传递）：
   - reviewer 是否**真看到 agent 提交的报告**（而不是看 `sanitized_detail_excerpt` 中上次自己的 detail）？
   - agent 报告是否**真到达** reviewer 的 review_artifacts？
   - 检测方法：看 reviewer 评判文本中是否引用 agent 当前报告的具体内容（路径名 / 关键词 / 诊断），而非引用之前 reviewer 自己的话

5. **后续动作**：
   - **Type 1 grant**：repair-gate.py 自动修复（装库 / lake update / 等）→ 自动返回原 state；审视修复是否真做了 + 返回后 agent 是否继续推进
   - **Type 2 grant**：human handoff → operator 接手；审视 operator 接手是否真发生
   - **Type 3 reject**：agent 是回到原 state 真做下一 move 还是又来一次相同 escape（后者 = 表演式应答）？
...
- **agent 想不出 workaround**（已穷尽 main flow 内可走的路径，每条都试过 + 写归因）
- **agent 对某类 tactic / unifier 的运用能力极限**（试了多种写法都撞 elaboration scope）

**v3 重分类（基于宪法 v7.0 精神 B）**：之前 v2 把"任务超过当前 mathlib API"列为模型能力上限——v3 移除这条。**mathlib 上游 API 缺失不是模型能力上限**：
- 库装错 / 依赖缺 → **PR type 1**（setup gap，REPAIR 自动修复）
- mathlib 上游真没有 API（mathlib master 也没）→ **PR type 2**（上游缺失，REPAIR 走 human handoff）
- 两者都是流程问题（流程没让 agent 合法报告需求），不是 capacity ceiling
- 把它归到 capacity 是给 agent 找借口
...
- **[v2 新增] 缺 task-infeasible 合法 sink**：sphere-packing-3 实证——agent best effort 撞 multi-week mathlib gap 后，G2（closure 二元）不接受任何 substantiated infeasibility 退出 → agent 自创工作流外通道（写 FINAL_SESSION_SUMMARY）—— 宪法 v7.0 PR 解决：上游真缺失走 PR type 2 → human handoff
...
- **[v3 新增] PI 通道结构性失效**（imo 实证 + sphere 实证）：`prep-escape-context.py` 的 `sanitized_detail_excerpt` 字段是**上次 reviewer 自己写的 detail 前 2000 字符**——reviewer 跟自己对话；agent 提交的内容没有任何字段路径能到 reviewer → reviewer 评判结构性退化为"reviewer 自言自语"。宪法 v7.0 PI 要求修：reviewer 必须看到 agent 真实提交的报告内容（不是看自己上一次的话）
...
| ID | 形态 | 实证 |
|---|---|---|
| **PX-1 [v2]** | **Agent 自创工作流外通道**：写 `FINAL_SESSION_SUMMARY.md` / `*_FINAL_*.md` / `comprehensive-handoff*.md` 等非 state-driven artifact，commit message 不属"S_X round N"格式。Counter 接近 threshold 时尤为高发——agent 自承"X/Y markers (threshold not reached)" 后 self-stop | **sphere-packing-3 b89bd23 "final session summary" commit 实证** |
...
| **PL-2 [v3]** | **Prompt 用通用 process 用语替代阶段精神**：如 "诚实推进 + 实质性策略选择"等抽象语言，不具体陈述该阶段精神（自由综合书写 / 弱点补充 / 专注执行 / 归因 + 机械结束）| 当前 yaml v6.8 实证（v7.0 应修） |
...
- **§2 各阶段补正面信号**（canada 实证）
- **§3.2 新增 H 类异常**：agent 写工作流外 artifact
- **§3.5 新增四 session 实证形态谱**
- **§4.5 新增"诚实最小满足"健康灰区**
- **§5.2/5.3 新增 task-infeasible sink + agent 行动质量 ≠ workflow 健康**
- **§6 anti-pattern 库 +6 条**：PA-6 / PA-7 / PR-6 / PV-7 / PV-8 / PX-1
- **§7 +2 条审视陷阱**：T12 / T13
- **§8.5 新增 Detection 时机要求**

**v3.0 (2026-05-11，宪法 v7.0 级联修订)**
- 宪法基础切换：v6.8 → **v7.0**（PA 重写为 escalation gate / PR / PI / PL 新立 / 精神 A+B + 5 条安全核心 / §八 测试分析原则）
- **§3.3 S_INFRA_REPAIR 模板重写**（v3 核心修订）：从"soft-gate coach"框架改为"PA 监督 + PR 三类诊断 + repair-gate.py 调度"框架；含 agent 尝试报告完整性审 / PR 诊断分流（type 1/2/3）/ reviewer 立场审视（监督鞭打而非心理按摩）/ PI 通道完整性审视
- **§5.1 mathlib 缺库重分类**：从"模型能力上限"→ PR type 1/2 处理对象；上游 API 缺失不是 capacity ceiling
