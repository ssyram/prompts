...
Given a codebase (or specific functions) WITHOUT an explicit design document, reverse-engineer a **descriptive design specification** by systematically examining **all usages** (call sites) of each target function. The output is a design document that captures what the code IS doing — not what it SHOULD do.
...
The method: for each function, examine **every call site** to understand what callers establish before calling and what callers expect after return. The function's Pre-conditions are what its callers collectively guarantee; its Post-conditions are what its callers collectively rely on.

A function that doesn't check X is not "missing a check" — if every caller already checks X before calling, then "caller guarantees X" IS the Pre-condition.

## Procedure

### Phase 1 — Per-function: Extract spec from implementation

For each function in scope, read ONLY its implementation and extract:
...
Internal guards (what the code validates / asserts / early-returns on):
  - (list each assert!, expect!, early-return guard, type constraint)
...
This is the critical step. For each function, **grep all call sites** and trace:

```
Call site: caller_module::caller_function, line N

What caller establishes before calling:
  - (what validation / state setup the caller does before this call)
...
Pre-conditions (intersection of what ALL callers guarantee):
  - (only list what EVERY caller establishes — the weakest common guarantee)

Post-conditions (union of what ANY caller relies on):
  - (list everything any caller depends on from the return)
...
From Phase 2's call-site analysis, derive:
...
Error philosophy: (fail-fast vs fail-soft vs fire-and-forget — derived from actual error handling patterns)
```

### Phase 4 — Identify intentional omissions

Scan for patterns that reveal **intentional design decisions**:

- A function that could validate X but doesn't, AND all callers validate X → "caller's responsibility by design"
- `let _ =` or `.ok()` on a Result → intentional error suppression (fire-and-forget)
- A function returning `T` (infallible) despite sub-operations that could fail → caller guarantees success conditions
- A function with a simpler contract than it "could" have → intentional simplicity trade-off
...
6. **Unvalidated Assumptions** — Pre-conditions that NO caller establishes (⚠ markers) — these are the only candidates for real bugs
...
- **Quote code, don't paraphrase.** Cite exact lines for each Pre-condition source.
